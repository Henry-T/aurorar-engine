...
namespace Aurora
{
	...
	{		
		...
		{
			os << "Aurora Memory: No memory leaks" << std::endl;
		}
		else
		{			
			os << "Aurora Memory: Detected memory leaks !!! " << std::endl;
			os << "Aurora Memory: (" << mAllocations.size() << ") Allocation(s) with total " << mTotalAllocations << " bytes." << std::endl;
			os << "Aurora Memory: Dumping allocations -> " << std::endl;
